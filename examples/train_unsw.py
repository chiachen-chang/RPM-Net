#!/usr/bin/env python3
"""
RPM-Net训练示例 - UNSW-NB15数据集

这个脚本展示了如何使用RPM-Net在UNSW-NB15数据集上进行训练和评估
"""
...
def main():
    ...
    print("🎯 开始RPM+Fisher在UNSW-NB15数据集上的完整实验")
    print("=" * 60)
    ...
    # 2. 加载数据
    data, class_info = load_data(config.DATA_DIR, dataset_name="unsw")
    ...
    # 8. 保存最终模型
    model_file = os.path.join(config.RESULT_DIR, 'rpm_fisher_model_final.pth')
    torch.save({
